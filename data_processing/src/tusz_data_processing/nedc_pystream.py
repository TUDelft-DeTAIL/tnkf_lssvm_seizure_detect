"""
    This script is from xx, and only used for testing purposes.
"""
...
def nedc_print_vals(fsamp_a, sig_a, labels_a):
    ...
    #
    if len(sig_a) == int(0):
        print("%s (%s: %s) empty signal" % (sys.argv[0], __name__, "nedc_print_vals"))
        return True
...
def nedc_load_parameters(pfile_a):
    ...
    for line in fp:
        ...
        #
            ...
            if parts[1] != KEYWORD_VERSION_NUMBER:
                print(
                    "%s (%s: %s): incorrect version number (%s)"
        ...
        #
        elif (str.startswith(keyword_upcase) == True) and (DELIM_BOPEN in str):
            flag_pblock = True
        ...
        #
            ...
            if parts[0] == KEYWORD_MONTAGE:
                values[parts[0]].append(parts[1].replace(DELIM_QUOTE, DELIM_NULL))

            # else: treat it as a normal name/value pair
            #
            else:
                values[parts[0]] = parts[1].replace(DELIM_QUOTE, DELIM_NULL)
    ...
    if flag_pblock == False:
        print(
            "%s (%s: %s): invalid parameter file (%s)"
            % (sys.argv[0], __name__, "nedc_load_parameters", pfile_a)
        )
        return None
...
def main(argv):
    ...
    #
    fsamp_sel, sig_sel, labels_sel = nedc_select_channels(params, fsamp, sig, labels)
